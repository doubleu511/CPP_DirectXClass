...
private:
    ...
    void BuildRenderItems();                // 렌더링할 기하도형
...
private:
    ...
    std::unordered_map<std::string, ComPtr<ID3D12PipelineState>> mPSOs;             // 렌더링 파이프라인 맵
...
void GeometryApp::Draw(const GameTimer& gt)
{
    ...
    mCurrFrameResource->Fence = ++mCurrBackBuffer;
...
void GeometryApp::BuildConstantBufferViews()
{
    ...
    // 각 프레임 차원의 물체마다 하나씩 CBV 서술자가 필요
...
void GeometryApp::BuildShadersAndInputLayout()
{
    ...
    HRESULT hr = S_OK;
    mShaders["standardVS"] = d3dUtil::CompileShader(TEXT("Shaders\\color.hlsl"), nullptr, "VS", "vs_5_1");
    mShaders["opaquePS"] = d3dUtil::CompileShader(TEXT("Shaders\\color.hlsl"), nullptr, "PS", "ps_5_1");
...
void GeometryApp::BuildPSO()
{
    ...
    psoDesc.VS =
    {
        reinterpret_cast<BYTE*>(mShaders["standerdVS"]->GetBufferPointer()), mShaders["standardVS"]->GetBufferSize()
    };
    psoDesc.PS =
    {
